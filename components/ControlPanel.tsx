  ...
  return (
    <div className="glass-card p-7 rounded-[2.5rem] shadow-2xl">
      <div className="space-y-7">
        ...
        {/* Source Image for Editing (Only in Edit Mode) */}
        {isEditMode && (
          <div>
            ...
            {!sourceImage ? (
              <button
                ...
                className="w-full border-2 border-dashed border-slate-700/40 rounded-3xl py-12 flex flex-col items-center justify-center text-slate-500 hover:border-indigo-500/50 hover:text-indigo-400 transition-all bg-slate-900/40 group"
              >
              ...
              <div className="relative rounded-3xl overflow-hidden group bg-slate-900 border border-white/5 shadow-2xl min-h-[120px] max-h-[300px] flex items-center justify-center p-2">
                <img 
                  src={sourceImage} 
                  alt="Source" 
        ...
        <div>
          <label className="block text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 ml-1 flex justify-between">
            ...
            <span className="text-[9px] text-indigo-400 font-bold">{isEditMode ? 'EDIT' : 'NEW'}</span>
          </label>
          <textarea
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            placeholder={isEditMode ? "How should we manifest the change?" : "A celestial dragon woven from starlight..."}
            className="w-full h-36 bg-slate-950/60 border border-white/5 rounded-[1.5rem] p-5 text-slate-100 placeholder-slate-700 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500/40 outline-none transition-all resize-none shadow-inner"
        ...
        <div>
          ...
          <div className="relative group">
            <select
              ...
              className="w-full bg-slate-950/60 border border-white/5 rounded-2xl p-4.5 text-slate-100 outline-none focus:ring-2 focus:ring-purple-500/20 transition-all cursor-pointer appearance-none shadow-inner"
              disabled={isGenerating}
            >
              ...
              <button
                ...
                className={`py-3 text-[10px] rounded-xl border transition-all duration-500 font-extrabold ${
                  aspectRatio === ratio
                    ...
                    : 'bg-slate-950/40 border-white/5 text-slate-600 hover:border-white/10 hover:text-slate-300'
                }`}
              >
                {ratio}
              </button>
