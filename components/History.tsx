...
const History: React.FC<HistoryProps> = ({ history, onClearHistory, onDeleteImage }) => {
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-slate-500 glass-card rounded-[3rem] border-dashed border-slate-800 reveal">
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center px-2 reveal">
        <h2 className="text-slate-400 text-xs font-black uppercase tracking-[0.3em]">
          Stored Manifestations ({history.length})
        </h2>
        <button
          onClick={onClearHistory}
          className="text-xs font-black uppercase tracking-[0.2em] text-red-400 hover:text-red-300 transition-colors flex items-center gap-2 group"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {history.map((img, index) => (
          <div 
            key={img.id} 
            className="bg-slate-900/50 backdrop-blur-md border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl group hover:border-indigo-500/30 transition-all duration-500 reveal"
          ...
          >
            <div className="aspect-square relative overflow-hidden bg-slate-950">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 backdrop-blur-[2px]">
                <button 
                  ...
                  className="p-4 bg-white/10 hover:bg-indigo-600 text-white rounded-2xl transition-all transform hover:scale-110 shadow-2xl"
                ...
                <button 
                  ...
                  className="p-4 bg-white/10 hover:bg-red-600 text-white rounded-2xl transition-all transform hover:scale-110 shadow-2xl"
            ...
            <div className="p-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[9px] font-black uppercase tracking-wider text-indigo-400 px-2.5 py-1 bg-indigo-400/10 rounded-full border border-indigo-400/20">
                  {img.style}
                </span>
                {img.isEdit && (
                  <span className="text-[9px] font-black uppercase tracking-wider text-pink-400 px-2.5 py-1 bg-pink-400/10 rounded-full border border-pink-400/20">
                    Transformed
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-slate-800/50 flex justify-between items-center">
                ...
                <span className="text-[9px] font-mono text-slate-700">{img.aspectRatio}</span>
              </div>
            </div>
          </div>
        ))}
